...
def info():
    ...
    ## Media Tools (VFX Workflows)
    - `create_foreground_plate(sequence_path, plate_name, camera_name=None, proxy_path=None, opacity_multiplier=1.0, emissive_multiplier=1.0, enable_loop=True)`
      Create complete foreground plate setup with alpha channel support
      - Creates ImgMediaSource, MediaPlayer, MediaTexture, VFX Material, ImagePlate
      - Material: Unlit, Masked blend (hard alpha), Two-Sided
      - Artist controls: OpacityMultiplier (ghosting), EmissiveMultiplier (brightness)
      - Use case: Set extensions with live action plates
    - `open_media_player(media_player_name, media_source_name=None)`
      Open and play a MediaPlayer with optional MediaSource
    - `list_image_sequences_in_folder(folder_path)`
      List all image sequences in a folder for batch operations
