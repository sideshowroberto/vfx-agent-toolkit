...
class HoudiniMCPServer:
    ...
    def _execute_command_internal(self, command):
        ...
        handlers = {
            ...
            "get_node_parameters": self.get_node_parameters,  # NEW: Query-first pattern
    ...
    def get_node_parameters(self, node_path):
        """
        Query all parameters of a node and return them as a structured list.
        This enables the "Query-First Pattern" to avoid parameter name errors.

        Returns parameter names, labels, types, and current values.
        """
        ...
        for parm in node.parms():
            try:
                ...
                params_list.append({
                    ...
                    "data_type": parm_template.dataType().name() if hasattr(parm_template, 'dataType') else None,
                    "current_value": str(parm.eval()),
                    "default_value": str(parm_template.defaultValue()) if hasattr(parm_template, 'defaultValue') else None,
                    "help": parm_template.help() if hasattr(parm_template, 'help') else "",
                })
            except Exception as e:
                # If any parameter causes an error, skip it but continue
                params_list.append({
                    "name": parm.name(),
                    "error": f"Could not read parameter: {str(e)}"
                })
    ...
    def _process_rendered_image(self, filepath, camera_path=None, view_name=None):
        """
        Helper to get metadata for a rendered image file.
        Returns filepath and metadata (Blender Bridge pattern - no base64 encoding).
        """
        ...
        result_data = {
            "status": "success",
            "filepath": filepath,  # Return filepath instead of base64
            "format": format,
