...
# -------------------------------------------------------------------
# Node Parameter Introspection Tool (Query-First Pattern)
# -------------------------------------------------------------------
@mcp.tool()
def get_node_parameters(ctx: Context, node_path: str) -> str:
    """
    Get all available parameters for a specific node in Houdini.
    Returns parameter names, labels, and types as JSON.
    Use this BEFORE attempting to set parameters to avoid errors.

    Args:
        node_path: Full path to the node (e.g., "/obj/geo1" or "/obj/AutoDopNetwork/staticobject1")

    Returns:
        JSON string containing parameter schema with name, label, and type information
    """
    try:
        conn = get_houdini_connection()
        response = conn.send_command("get_node_parameters", {"node_path": node_path})

        if response.get("status") == "error":
            origin = response.get('origin', 'houdini')
            return f"Error ({origin}): {response.get('message', 'Unknown error')}"

        # Return formatted parameter information
        params_data = response.get("result", {})
        return json.dumps(params_data, indent=2)

    except ConnectionError as e:
        return f"Connection Error getting node parameters: {str(e)}"
    except Exception as e:
        logger.error(f"Unexpected error in get_node_parameters tool: {str(e)}", exc_info=True)
        return f"Server Error retrieving node parameters: {str(e)}"
...
def render_single_view(ctx: Context,
    ...
    Returns filepath for Claude to Read and view the image.
    """
    try:
        conn = get_houdini_connection()
        ...
        if isinstance(result, dict):
            # Format response similar to Blender Bridge
            ...
            return json.dumps({
                ...
                "message": f"Render saved to {filepath}"
            }, indent=2)
        return "Render completed but no output path returned."
    except Exception as e:
...
def render_quad_views(ctx: Context,
    ...
    Returns filepaths for Claude to Read and view the images.
    """
    try:
        conn = get_houdini_connection()
        ...
        if isinstance(result, dict) and "results" in result:
            # Format multiple render results
            renders = []
            ...
            return json.dumps({
                ...
                "renders": renders,
                "count": len(renders),
                "message": f"Rendered {len(renders)} views to {render_path}"
            }, indent=2)
        return "Render completed but no output returned."
    except Exception as e:
