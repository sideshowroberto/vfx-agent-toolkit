# unreal-vfx installer (vfx-agent-toolkit)
# Delegates to the unreal MCP connector, which installs the bridge and
...
$connector = Join-Path $PSScriptRoot "..\..\connectors\unreal\install.ps1"
if (-not (Test-Path $connector)) {
    # register.ps1-only connectors (no bridge payload)
    $connector = Join-Path $PSScriptRoot "..\..\connectors\unreal\register.ps1"
}
if (Test-Path $connector) {
    & $connector @args
    exit $LASTEXITCODE
} else {
    Write-Host "Connector not found: connectors\unreal" -ForegroundColor Red
    ...
    Write-Host "  vfx-agent-toolkit\connectors\unreal\install.ps1" -ForegroundColor Cyan
    exit 1
}
