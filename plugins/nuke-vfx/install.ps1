# nuke-vfx installer (vfx-agent-toolkit)
# Delegates to the nuke MCP connector, which installs the bridge and
...
$connector = Join-Path $PSScriptRoot "..\..\connectors\nuke\install.ps1"
...
if (Test-Path $connector) {
    ...
    Write-Host "Connector not found: connectors\nuke" -ForegroundColor Red
    ...
    Write-Host "  vfx-agent-toolkit\connectors\nuke\install.ps1" -ForegroundColor Cyan
    exit 1
}
